...
XmlReaderError::XmlReaderError(const string& m) : Error(m) {}
...
const string& XmlNode::getChildValue(const string& key) const {
    ...
    throw XmlReaderError("tag [" + name + "] on line " + itoa(line) + " has no child '" + key + "'");
}
...
const string& XmlNode::getAttribute(const string& key) const {
    ...
    if (iter == attributes.end()) throw XmlReaderError("tag [" + name + "] on line " + itoa(line) + " has no attribute '" + key + "'");
    return iter->second;
}
        ...
        : root(0), line(1), inData(false), inComment(false)
{
    ...
    if (file == 0) throw XmlReaderError(string("error opening file: ") + filename);
...
void XmlReader::foreach(const std::string& path, XmlNodeVisitor& visitor) const {
    XPath xpath(path);
    if (xpath.size() < 2) {
        throw XmlReaderError("invalid path [" +  path + "]");
    }
    if (root->getName() != xpath[0]) {
        throw XmlReaderError(string("no such root [") + xpath[0] + "]");
    }
    const XmlNode* node = root;
    for (int i=1; i<xpath.size()-1; i++) {
        const XmlNode* child = node->getChild(xpath[i]);
        if (child == NULL) {
            throw XmlReaderError("node [" + node->getName() + "] has no child [" + xpath[i] + "]");
        }
        node = child;
    }
...
void XmlReader::error(const string& msg) {
    delete root;
    root = 0;
    throw XmlReaderError(msg + " (line: " + itoa(line) + ")");
}
